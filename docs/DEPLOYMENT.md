...
| `OPENROUTER_API_KEY`                            | api, worker | OpenRouter auth                                                                                | Required for M3+ (audits) and M4 (email drafting)                                                                                       |
...
| `BUSINESS_CONTEXT`                              | api         | Who you are / ideal customer, injected into `business-audit-v1` (see [PROMPTS.md](PROMPTS.md)) | Set to your real business/ICP description                                                                                               |
| `AUDIT_MAX_CONCURRENCY`                         | api         | Simultaneous AI audits                                                                         | Tune to OpenRouter rate limits                                                                                                          |
| `OUTREACH_SENDER_NAME`                          | api         | `{{senderName}}` in the outreach email template                                                | Set to your real name/team                                                                                                              |
| `OUTREACH_EMAIL_TEMPLATE`                       | api         | Static email body wrapping the model's `{{opener}}`                                            | Customize copy/CTA/signature                                                                                                            |
| `EMAIL_DRAFT_MAX_CONCURRENCY`                   | api         | Simultaneous email drafts                                                                      | Tune to OpenRouter rate limits                                                                                                          |
| `N8N_API_BASE_URL`                              | api         | URL the API calls to trigger n8n workflows                                                     | `http://n8n:5678` inside Compose (already set); public URL if n8n moves off-host                                                        |
| `N8N_WEBHOOK_SECRET`                            | api, n8n    | Shared secret for `X-Webhook-Secret`, both directions                                          | 32+ random bytes; **must match exactly** on the api container and inside the imported n8n workflows                                     |
| `API_BASE_URL`                                  | n8n         | API's address as reachable from n8n (`$env.API_BASE_URL` in the workflows)                     | `http://api:4000/api` inside Compose (already set); public URL if api moves off-host                                                    |
| `OUTREACH_FROM_EMAIL`                           | n8n         | "From" address for outreach emails                                                             | Your operator email; the actual send credential is configured in the n8n UI (see [`n8n/README.md`](../n8n/README.md)), not this env var |
| `YELP_BASE_URL`                                 | api         | Directory site the lead-discovery module scrapes                                               | Default fine; only change if scraping is ported to a different directory                                                                |
| `LEAD_DISCOVERY_MAX_CONCURRENCY`                | api         | Simultaneous lead-discovery searches                                                           | Lower than the other modules' default — scraping trips anti-bot defenses more easily under concurrency                                  |
| `LEAD_DISCOVERY_NAVIGATION_TIMEOUT_MS`          | api         | Max time to load a search-results or detail page                                               | Default `15000` fine                                                                                                                    |
| `N8N_PORT` / `N8N_HOST` / `N8N_PROTOCOL`        | n8n         | Public identity of the n8n instance                                                            | Public HTTPS values                                                                                                                     |
| `WEBHOOK_URL`                                   | n8n, api    | Base URL for n8n's own inbound webhooks                                                        | Public HTTPS URL                                                                                                                        |
